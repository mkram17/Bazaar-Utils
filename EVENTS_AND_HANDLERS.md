...
- [`@OnlyBazaarScreen(...)`](src/main/java/com/github/mkram17/bazaarutils/events/predicates/OnlyBazaarScreen.java) — runs only on matching bazaar screens: an explicit `value` whitelist, `any = true`, or `useConstrainsInterface = true` to delegate to the instance's `ScreenConstrained`.
