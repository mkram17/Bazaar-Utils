...
public sealed interface BookMutation<O extends BazaarDataOrigin> permits
    ...
    record Decrement<O extends BazaarDataOrigin.UserPositionEvent>(
        ...
        public MutationOutcome apply(String productId, O origin) {
            ...
            // Attributed on intent, not on what the level absorbed. The claim is that this
            // volume's fill is already credited to an order, which holds whether or not the
            // book was in a state to record the decrement.
            return new MutationOutcome(changed, data, List.of(), List.of(),
                    cause == Cause.FILL ? AttributedFills.of(type, price, amount) : AttributedFills.NONE);
        }
    }
    ...
    record At<O extends BazaarDataOrigin>(O origin, BookMutation<O> inner) implements BookMutation<O> {
        @Override
        public MutationOutcome apply(String productId, O ignored) {
    ...
    record MutationOutcome(
            ...
            List<AbsenceScope> instabuyEvictions,
            List<AbsenceScope> instasellEvictions,
        ...
        static MutationOutcome confirmed(boolean changed, ProductData data) {
            return new MutationOutcome(changed, data, List.of(), List.of(), AttributedFills.NONE);
        }

        /** The product was never registered and this mutation had no standing to create it. */
        static MutationOutcome unregistered() {
            return new MutationOutcome(false, null, List.of(), List.of(), AttributedFills.NONE);
        }
        ...
        static MutationOutcome withEviction(boolean changed, ProductData data, PriceType type, AbsenceScope scope) {
            return type == PriceType.INSTABUY
                    ? new MutationOutcome(changed, data, List.of(scope), List.of(), AttributedFills.NONE)
                    : new MutationOutcome(changed, data, List.of(), List.of(scope), AttributedFills.NONE);
        }
        ...
        public boolean canEvict(PriceType type, double price) {
            return (type == PriceType.INSTABUY ? instabuyEvictions : instasellEvictions)
                    .stream()
                    .anyMatch(scope -> scope.contains(type, price));
        }

        MutationOutcome merge(MutationOutcome next) {
            return new MutationOutcome(
                    changed || next.changed(),
                    next.book() != null ? next.book() : book,
                    union(instabuyEvictions, next.instabuyEvictions()),
                    union(instasellEvictions, next.instasellEvictions()),
                    attributed.merge(next.attributed()));
        }

        private static List<AbsenceScope> union(List<AbsenceScope> prior, List<AbsenceScope> next) {
            if (next.isEmpty()) return prior;
            if (prior.isEmpty()) return next;

            var merged = new ArrayList<AbsenceScope>(prior.size() + next.size());
