...
public final class OrderCancelledDataSource extends ChatOrderSource {
    ...
    private void onBuyOrderCancelled(BazaarChatEvent.BuyOrderCancelled event) {
        var transaction = TransactionType.BUY_ORDER;
        ...
        PlayerActionUtil.notifyAll("%s — Cancelled: %s".formatted(origin.describe(), cancelled.describe()), NotificationType.ORDERDATA);
        ...
        var mutation = BookMutation.withdrawn(transaction, matched.pricePerItem(), matched.unfilledAmount());
        var delta = new OrderDelta.Evict<>(matched, cancelled, mutation);
    ...
    private void onSellOfferCancelled(BazaarChatEvent.SellOfferCancelled event) {
        ...
        var matched = OrderResolver.forSellCancel(productId, event.amount, storage).orElse(null);
        if (matched == null) {
            Util.logMessage("Sell cancel skipped (already screen-reconciled) — productId=%s".formatted(productId));
        ...
        PlayerActionUtil.notifyAll("%s — Book decrement: %s %s Δ%d @ %.4f (cancelled)".formatted(
                origin.describe(),
                transaction.getPriceType(),
                productId, matched.unfilledAmount(), matched.pricePerItem()), NotificationType.BAZAARDATA);
