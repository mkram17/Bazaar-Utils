...
/**
 * Source and timestamp of a market book write, governing eviction authority in
 * {@link com.github.mkram17.bazaarutils.data.bazaar.book.ProductData}.
 *
 * <p>{@link Snapshot} sources are authoritative reads — they may evict stale levels
 * within their observed scope. {@link UserPositionEvent} sources are optimistic point
 * mutations — they never assert absence.
 */
public sealed interface BazaarDataOrigin permits BazaarDataOrigin.Snapshot, BazaarDataOrigin.UserPositionEvent {
    ...
    /**
     * An order has lapsed its 7-day Hypixel lifetime. Detected by {@code OrderExpiryDataSource}
     * on a per-minute cron, or by {@code OrdersScreenDataSource} on screen observation via the
     * {@code §8Expired!} lore token. The {@link #timestamp()} is the detection time, not the
     * exact moment the order expired.
     */
    record OrderExpired(long timestamp) implements UserPositionEvent {
        @Override
        public String describe() {
            return "Order expiry @ " + timestamp;
        }
    }
    ...
    /**
     * Reconciliation from one Orders screen render. Authoritative for fill progress, claim
     * progress, slot positions, and order existence as of {@link #observedAt}. Not a market
     * snapshot — does not evict book levels beyond the decrements implied by fills and
     * cancellations on user-owned positions.
     */
