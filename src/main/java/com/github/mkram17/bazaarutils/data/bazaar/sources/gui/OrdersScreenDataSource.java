...
public final class OrdersScreenDataSource extends BUListener {
    ...
    public void onContainerLoaded(ContainerLoadedEvent event) {
        ...
        var committed = commitAll(staged, origin, key, observed);

        RenderedOrdersIndex.update(
                onScreenBySlot,
                committed != null ? committed : storage,
                key);
    }
    ...
    private static @Nullable Order findMatch(Map<UUID, Order> candidates, Set<UUID> usedIds, PageOrderParser.ParsedEntry entry) {
        ...
        Predicate<Order> exactPrice = Order.forPrice(price);
        ...
        Predicate<Order> compatible = order ->
                !usedIds.contains(order.id())
                        ...
                        && Order.forPrice(info).test(order)
    ...
    private static Order reconcileExisting(
        ...
        boolean reconciledPriceExact = found.priceExact()
                || Order.forPrice(entry.info().getPricePerItem()).test(found);
        ...
        if (found.slotPosition().isOnScreenAt(entry.item().slotIndex())
                ...
                && Order.forPrice(reconciledPrice).test(found)
     ...
     */
    private @Nullable List<Order> commitAll(List<StagedCommit> staged, BazaarDataOrigin.OrdersScreen origin, ProfileKey key, Set<UUID> observed) {
        if (staged.stream().noneMatch(StagedCommit::changed)) return null;
        ...
        var confirmed = Stream.concat(
                        staged.stream().flatMap(commit -> commit.confirmed().stream()),
                        observed.stream())
                .collect(Collectors.toUnmodifiableSet());

        var result = FillInference.applyAndSettle(key, outcomes, origin, ignored -> next, confirmed);
        ...
        return result.ordersOrElse(key, next);
    }
