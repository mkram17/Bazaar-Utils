...
public final class OrdersScreenDataSource extends BUListener {
     ...
     *
     * <p>{@code isKnownCoop} for this parse is the persisted flag OR this container's
     * own title being the co-op variant — a live, this-tick signal on top of whatever
     * was already recorded, since the persisted flag might not have caught up yet. If
     * the parse itself surfaces any coop-attributed entry and the profile wasn't
     * already marked coop, it's marked now — a screen render can prove coop status the
     ...
     */
    private StagedCommit reconcileProduct(ProductMatch match, Map<Integer, Order> onScreenBySlot, BazaarDataOrigin.OrdersScreen origin, ProfileKey key) {
        ...
        // Deltas produced this reconciliation — Place, Evict, Update, PriceCorrection,
        // and Reanchor entries. Floor affirmation (Phase 3d) mutates the book directly
        // via `mutation` and never appears here as a delta.
        ...
        for (var kv : diff.entriesOnlyOnLeft().entrySet()) {
            ...
            if (futureDated) {
            ...
            } else if (order.isActionable() && (sibling != null || (origin.observedAt() - order.lastUpdatedAt()) > EVICTION_GRACE_MS)) {
                ...
                if (order.isFilled()) {
                    ...
                    // Filled and gone without ever being claimed here — presumed auto-claimed.
                    // Two distinct, unverified explanations both lead to the same outcome (a
                    // coop peer claimed it, or a different session of the same player did),
                    // and neither depends on this order's own OrderAttribution — that field
                    // answers who placed it, not who could have acted on it since. No
                    // distinction is drawn between the two; both just auto-claim.
                    ...
                    deltas.add(new OrderDelta.Evict<>(order, order.cancelled(origin), BookMutation.none()));
                    ...
                    BookMutation<BazaarDataOrigin.OrdersScreen> cancelMutation = BookMutation.decrement(
                    ...
                    deltas.add(new OrderDelta.Evict<>(order, order.cancelled(origin), cancelMutation));
                ...
                var demoted = order.reanchored(new OrderSlotPosition.OffScreen(0));
                deltas.add(new OrderDelta.Reanchor<BazaarDataOrigin.OrdersScreen>(order, demoted));
        ...
        for (var entry : diff.entriesDiffering().entrySet()) {
            ...
            if (Double.compare(before.pricePerItem(), after.pricePerItem()) != 0) {
                ...
                BookMutation<BazaarDataOrigin.OrdersScreen> repriceMutation = BookMutation.<BazaarDataOrigin.OrdersScreen>decrement(TransactionType.of(before.side(), TransactionType.Method.ORDER),
                ...
                BookMutation<BazaarDataOrigin.OrdersScreen> fillMutation = BookMutation.decrement(
                        TransactionType.of(after.side(), TransactionType.Method.ORDER),
            ...
            } else if (after.hasExpired() && !before.hasExpired()) {
                ...
                // this tick.
                BookMutation<BazaarDataOrigin.OrdersScreen> expiryMutation = BookMutation.decrement(
                        TransactionType.of(after.side(), TransactionType.Method.ORDER),
                        after.pricePerItem(),
        ...
        for (var side : TransactionType.Side.values()) {
            ...
            var levels = reconciledById.values().stream()
                    ...
                    .<PriceLevel<BazaarDataOrigin.UserPositionEvent>>map(order -> new PriceLevel<>(order.pricePerItem(), order.unfilledAmount(), 1, origin))
                    .toList();
    ...
    private static @Nullable Order findMatch(Map<UUID, Order> candidates, Set<UUID> usedIds, PageOrderParser.ParsedEntry entry) {
        ...
        return candidates.values().stream()
                ...
                .min(oldestFirst)
                .or(() -> candidates.values().stream().filter(base.and(Order::isActionable)).min(oldestFirst))
                ...
                // isCoopContext() — a SelfInCoop or CoopPeer order is always screen-sourced
                ...
                // attribution reconciles into whichever this screen confirms — SelfInCoop
                // if it's this same account from an untracked session, CoopPeer if it's
    ...
    private static Order reconcileExisting(Order found, PageOrderParser.ParsedEntry entry, BazaarDataOrigin.OrdersScreen origin) {
        ...
        // Reconciles the FillState shape first — from reconciledFill alone,
        // independent of whether entry.expired() also fires this tick — then wraps
        // it in Expired only if needed. found.status().fillState() looks through an
        // already-Expired found the same way a bare FillState found does, so "was
        // this order already expired last tick" needs no separate carry-through.
        // Target shape decided by PageOrderParser.resolveFillState, fed the
        ...
        var priorFillState = found.status().fillState();
        var targetShape = PageOrderParser.resolveFillState(reconciledFill, entry.info().getVolume(), origin.observedAt());
        OrderStatus.FillState reconciledFillProgress = switch (targetShape) {
            case OrderStatus.Filled fresh -> priorFillState.filter(it -> it instanceof OrderStatus.Filled).orElse(fresh);
            case OrderStatus.Partial fresh -> priorFillState.filter(it -> it instanceof OrderStatus.Partial)
                    ...
                    .map(existing -> reconciledFill > found.filledAmount()
                            ...
                            : existing)
                    .orElse(fresh);
            case OrderStatus.Set fresh -> priorFillState.filter(ns -> ns instanceof OrderStatus.Set).orElse(fresh);
        };
        ...
        if (entry.expired()) {
            ...
            reconciledStatus = new OrderStatus.Expired(expiredAt, reconciledFillProgress);
        } else {
            reconciledStatus = (OrderStatus) reconciledFillProgress;
        }
    ...
    private static long resolvePlacedAt(PageOrderParser.ParsedEntry entry, BazaarDataOrigin.OrdersScreen origin, long staggerOffset) {
        long base = entry.expiresAt().isPresent()
     ...
     */
    private record StagedCommit(String productId, List<Order> orders, List<UserOrderEvent> events, BookMutation.MutationOutcome outcome, boolean changed) {}
     ...
     *                  Update, PriceCorrection, and Reanchor entries only. BookOnly and
    ...
    private StagedCommit stageCommit(
        ...
        for (var delta : deltas) {
            switch (delta) {
                ...
                case OrderDelta.PriceCorrection<BazaarDataOrigin.OrdersScreen> correction -> {
        ...
        return new StagedCommit(productId, next, events, outcome, !deltas.isEmpty() || bookChanged);
    }
    ...
    private void commitAll(List<StagedCommit> staged, BazaarDataOrigin.OrdersScreen origin, ProfileKey key) {
        ...
        FillInference.applyAndSettle(key, outcomes, origin, ignored -> next);
