...
public final class OrderPlacedDataSource extends ChatOrderSource {
    ...
    public void onBuyOrderCreated(BazaarChatEvent.BuyOrderCreated event) {
        ...
        var key = ProfileKey.requireProfile(origin.describe());
        if (key == null) return;

        var reference = OrderResolver.resolveForPlacement(
                event.amount,
                Util.truncateNum(event.totalCoins / event.amount));

        applyPlacement(event.product, TransactionType.BUY_ORDER, key, origin, reference);
    }
    ...
    public void onSellOfferCreated(BazaarChatEvent.SellOfferCreated event) {
        ...
        var reference = OrderResolver.resolveForPlacement(
                event.amount,
                Util.truncateNum(event.totalCoins / event.amount) / ((100.0 - tax) / 100.0));

        applyPlacement(event.product, TransactionType.SELL_OFFER, key, origin, reference);
    }
     ...
     */
    private void applyPlacement(@NotNull String productDisplayName,
                                @NotNull TransactionType transaction,
                                @NotNull ProfileKey key,
                                @NotNull BazaarDataOrigin.OrderPlaced origin,
                                @NotNull PriceQuantity reference) {
