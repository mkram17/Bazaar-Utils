...
/**
 * The complete outcome of a source's resolution phase for a single Bazaar interaction.
 *
 * <p>Each variant maps to a distinct combination of storage write, book mutation, and
 * {@link com.github.mkram17.bazaarutils.events.bazaar.UserOrderEvent} type. Sources produce
 * a delta and hand it to
 * {@link com.github.mkram17.bazaarutils.data.bazaar.pipeline.ChatOrderSource#commit} or
 * {@link com.github.mkram17.bazaarutils.data.bazaar.pipeline.SnapshotSource#commitAll};
 * nothing past that point is the source's concern.
 *
 * <p>{@link #mutation()} defaults to {@link BookMutation#NONE} so that commit paths can
 * compose all mutations in a single reduction pass without switching on type.
 */
public sealed interface OrderDelta permits
    ...
    record Update(
        ...
        /** Convenience factory for expiry transitions. Carries the book decrement mutation. */
        public static Update expiry(Order before, Order after, BookMutation mutation) {
        ...
        public enum UpdateKind {
            ...
            /**
             * Order transitioned to {@link OrderStatus.Expired}.
             * Carries a {@link BookMutation#decrement} for the unfilled volume —
             * Hypixel removed it from their market book when the order lapsed.
             * Fires {@link UserOrderEvent.Expired}.
             */
            EXPIRY {
                @Override
                public UserOrderEvent getEvent(Order before, Order after) {
                    return new UserOrderEvent.Expired(after);
                }
            },
