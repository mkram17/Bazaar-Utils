...
public sealed interface OrderDelta<O extends BazaarDataOrigin> permits
    ...
    /**
     * The ID of an order this delta just confirmed a fill amount for directly — not by
     * inference — or {@code null} if this delta didn't. Feeds
     * {@link BookMutation.AttributedFills} so the same fill volume can't be
     * misattributed a second time by {@link FillInference} scanning the same level's
     * remaining occupants.
     */
    default @Nullable UUID confirmedFill() {
        return null;
    }

    /** Collects every non-null {@link #confirmedFill()} in {@code deltas}. */
    /** Collects every non-null {@link #confirmedFill()} across {@code deltas} into one set. */
    static Set<UUID> confirmedFills(Collection<? extends OrderDelta<?>> deltas) {
        return deltas.stream()
                .map(OrderDelta::confirmedFill)
    ...
    record Place<O extends BazaarDataOrigin>(Order order, BookMutation<O> mutation) implements OrderDelta<O> {
        ...
        /**
         * The order's own ID, when a crossed placement leaves it still open with some
         * fill already credited — {@code null} for an uncrossed placement (nothing to
         * attribute) or a fully-crossed one (the order has already left the book, so
         * there's no remaining level occupancy left to protect against double-counting).
         */
        @Override
        public @Nullable UUID confirmedFill() {
            return order.isOpen() && order.filledAmount() > 0 ? order.id() : null;
        }
    ...
    record Update<O extends BazaarDataOrigin>(
        ...
        /**
         * The order's own ID, when this is a fill advance that leaves it still open —
         * {@code null} for anything else, including a fill that just completed the
         * order, since a completed order has already left the book.
         */
        @Override
        public @Nullable UUID confirmedFill() {
            return kind == UpdateKind.FILL && after.isOpen() ? after.id() : null;
        }
        ...
        public static <O extends BazaarDataOrigin> Update<O> expiry(Order before, Order after, BookMutation<O> mutation) {
            return new Update<>(before, after, mutation, UpdateKind.EXPIRY);
        }
        ...
        public enum UpdateKind {
            ...
            FILL {
                ...
                public UserOrderEvent getEvent(Order before, Order after, ProfileKey key) {
                    boolean fullyFilled = after.isFilled();
    ...
    /**
     * Order evicted from storage. {@code before} is its state immediately prior;
     * {@code after} is what actually gets removed and reported in the
     * {@link UserOrderEvent.Cancelled} this fires. {@code mutation} carries whatever
     * book decrement accompanies the removal — {@link BookMutation#none()} when the
     * book side was already cleared by an earlier event.
     */
    record Evict<O extends BazaarDataOrigin>(Order before, Order after, BookMutation<O> mutation) implements OrderDelta<O> {
        public String productId() {
            return after.productId();
        }
    }
    ...
    /** Screen-observed data corrections*/
    record DataCorrection<O extends BazaarDataOrigin>(Order before, Order after, BookMutation<O> mutation) implements OrderDelta<O> {
        public String productId() {
            return before.productId();
        }
    }
     ...
     */
    record Reanchor<O extends BazaarDataOrigin>(Order before, Order after) implements OrderDelta<O> {
        public String productId() {
            return before.productId();
        }
    }
