...
public abstract class ChatOrderSource extends BUListener {
    ...
    protected final void commit(OrderDelta<BazaarDataOrigin.UserPositionEvent> delta, BazaarDataOrigin.UserPositionEvent origin) {
        switch (delta) {
            ...
            case OrderDelta.BookOnly<BazaarDataOrigin.UserPositionEvent> book -> {
                ...
                FillInference.settle(List.of(outcome), origin, FillInference.Eligibility.ALWAYS);
     ...
     * mutation, writes storage via {@link FillInference#applyAndSettle} — netting the
     * delta's own {@link OrderDelta#confirmedFills} out of that pass's shortfall
     * calculation, so a fill this delta already confirms can't also be inferred onto a
     * sibling resting at the same price — fires whichever {@link UserOrderEvent} the
     * variant implies, tagged with {@code key} as supplied here rather than resolved
     * internally, and posts {@link BazaarDataUpdateEvent}.
     */
    protected final void commit(OrderDelta<BazaarDataOrigin.UserPositionEvent> delta, BazaarDataOrigin.UserPositionEvent origin, @NotNull ProfileKey key) {
        ...
        var confirmed = OrderDelta.confirmedFills(List.of(delta));
        ...
        switch (delta) {
            ...
            case OrderDelta.Update<BazaarDataOrigin.UserPositionEvent> update -> {
                ...
                var resolved = UserOrdersStorage.findById(result.ordersOrElse(key, List.of()), update.before().id())
                        .orElse(update.after());
            ...
            case OrderDelta.Swap<BazaarDataOrigin.UserPositionEvent> swap -> {
                ...
                var inserted = UserOrdersStorage.findById(result.ordersOrElse(key, List.of()), swap.newSell().id()).orElse(swap.newSell());
