...
/**
 * Splices one product page's rendered price levels into the book, firing on every
 * Product Page container load.
 */
@DataSource
public final class PageSummaryDataSource extends SnapshotSource {
     ...
     * parses their lore into ask and bid levels, and splices them via {@link #splice}.
     *
    ...
    public void onContainerLoaded(ContainerLoadedEvent event) {
        ...
        var buyStack = ProductPageLayout.getCreateBuyOrderItem(context)
                .map(ItemInfo::itemStack)
                .orElse(null);

        var sellStack = ProductPageLayout.getCreateSellOfferItem(context)
                .map(ItemInfo::itemStack)
                .orElse(null);

        if (buyStack == null || sellStack == null) return;

        var parsed = PageSummaryParser.parseItemPage(buyStack, sellStack);

        var asks = parsed.askLevels().stream()
                .map(level -> new PriceLevel<>(level.pricePerUnit(), level.volume(), level.orderCount(), origin))
                .toList();

        var bids = parsed.bidLevels().stream()
                .map(level -> new PriceLevel<>(level.pricePerUnit(), level.volume(), level.orderCount(), origin))
                .toList();

        var book = new BookLevels<>(asks, bids);
        var outcome = splice(
                productId,
                book.asksLevels(),
                book.bidsLevels(),
        ...
        boolean changed = outcome.changed() || !FillInference.settle(List.of(outcome), origin, FillInference.Eligibility.ALWAYS).changedProducts().isEmpty();

        PlayerActionUtil.notifyAll(
                "%s: %s — %d buy levels, %d sell levels".formatted(
                        origin.describe(),
                        productId,
                        book.asksLevels().size(),
                        book.bidsLevels().size()
                ),
        ...
        Util.logMessage(
                "%s: %s buyBook=%s sellBook=%s".formatted(
                        origin.describe(),
                        productId,
    ...
    /** Formats the top-of-book price in {@code book}, or {@code "N/A"} if empty. */
    private static String bestPrice(NavigableMap<Double, PriceLevel<?>> book) {
        var entry = book.firstEntry();

        return entry == null
                ? "N/A"
                : String.valueOf(entry.getValue().pricePerUnit());
    }
}