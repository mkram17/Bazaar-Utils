...
public final class PageSummaryDataSource extends SnapshotSource {
     ...
     * parses their lore into ask and bid levels, splices them via {@link #splice},
     * then infers and applies fill progress for every known profile — trivially,
     * since exactly one product arrives per load here, there's nothing to
     * accumulate across.
     *
     ...
     * storage. {@link BazaarDataUpdateEvent} is posted only when the splice or any
     * profile's inference actually changed something.
     */
    ...
    public void onContainerLoaded(ContainerLoadedEvent event) {
        ...
        boolean changed = outcome.changed() || !FillInference.settle(List.of(outcome), origin, FillInference.Eligibility.ALWAYS).isEmpty();

        PlayerActionUtil.notifyAll(
                "%s: %s — %d buy levels, %d sell levels".formatted(
                        origin.describe(),
                        productId,
                        parsed.asksLevels().size(),
                        parsed.bidsLevels().size()
                ),
