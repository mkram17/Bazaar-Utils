...
public final class ProductPageLayout {
    ...
    public static Optional<ItemInfo> getCreateBuyOrderItem(@NotNull ScreenContext context) {
        return ScreenSwitch.<Optional<ItemInfo>>on(context)
                .when(BazaarScreenType.PRODUCT_PAGE, ctx -> SlotLookup.getInventoryItem(ctx, BazaarSlots.PRODUCT_PAGE.CREATE_BUY_ORDER.slot))
                .orElse(Optional.empty());
    }

    public static Optional<ItemInfo> getCreateSellOfferItem(@NotNull ScreenContext context) {
        return ScreenSwitch.<Optional<ItemInfo>>on(context)
                .when(BazaarScreenType.PRODUCT_PAGE, ctx -> SlotLookup.getInventoryItem(ctx, BazaarSlots.PRODUCT_PAGE.CREATE_SELL_OFFER.slot))
                .orElse(Optional.empty());
    }

    public static Optional<ItemInfo> getDisplayItem(@NotNull ScreenContext context) {
        return ScreenSwitch.<Optional<ItemInfo>>on(context)
                .when(BazaarScreenType.PRODUCT_PAGE, ctx -> SlotLookup.getInventoryItem(ctx, BazaarSlots.PRODUCT_PAGE.ITEM_DISPLAY.slot))
                .orElse(Optional.empty());
    }
