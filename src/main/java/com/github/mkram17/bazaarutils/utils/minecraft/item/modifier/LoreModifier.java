...
public interface LoreModifier extends AbstractItemModifier {
    ...
    /**
     * Appends the modification indicator to {@code line} when the resolved placement
     * for this modifier is {@link ModifyIndicator.IndicatorPlacement.AtModification}.
     * <p>
     * Safe to call unconditionally — degrades to a pass-through for every other
     * config value, so feature code never needs to branch on global config state.
     */
    default Component withAtModificationIndicator(Component line) {
        if (BUConfig.MODIFY_INDICATOR.resolve(this) instanceof ModifyIndicator.IndicatorPlacement.AtModification(
                boolean prefix
        )) {
            return prefix
                    ? Component.empty().append(ModifyIndicator.INDICATOR_WITH_SPACE).append(line)
                    : Component.empty().append(line).append(ModifyIndicator.SPACE_WITH_INDICATOR);
        }
