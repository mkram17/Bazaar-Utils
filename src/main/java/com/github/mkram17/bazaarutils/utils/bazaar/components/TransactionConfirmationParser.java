...
public final class TransactionConfirmationParser {
    ...
    private static final Pattern FLIP_PRICE_PATTERN = Pattern.compile("(?<![\\d.])(?<price>\\d+(?:\\.\\d)?)(?![\\d.])");
    ...
    public static Optional<Double> parseFlipPrice(@NotNull String[] lines) {
        String folded = String.join(" ", lines);
        Matcher matcher = FLIP_PRICE_PATTERN.matcher(folded);

        while (matcher.find()) {
            try {
                double price = Double.parseDouble(matcher.group("price"));

                if (price >= PriceInfo.MINIMUM_PRICE) {
    ...
    private static Optional<OrderInfo> parse(@Nullable ItemStack itemStack, TransactionType.Side side, Pattern amountPattern) {
        ...
        try {
            ...
            return OrderInfo.of(product, side, price, volume, true);
