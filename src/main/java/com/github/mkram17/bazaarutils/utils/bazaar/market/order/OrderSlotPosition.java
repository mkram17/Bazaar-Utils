...
public sealed interface OrderSlotPosition permits
    ...
    /** No longer part of any queue, on-screen or off — the order is {@link OrderStatus.Terminal}. */
    record Terminal() implements OrderSlotPosition {
        static final MapCodec<Terminal> CODEC = MapCodec.unit(Terminal::new);
    }
    ...
    default String describe() {
        return switch (this) {
            case OnScreen(int slot) -> "slot=on-screen[pos=" + slot + "]";
            case OffScreen(int logicalPos) -> "slot=off-screen[pos=" + logicalPos + "]";
            case Terminal ignored -> "slot=terminal";
        };
    }
}