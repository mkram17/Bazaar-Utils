...
public final class PageOrderParser {
    ...
    private static @Nullable ParsedEntry parseEntry(ItemInfo item, ProfileKey key, String localPlayerName, boolean isKnownCoop, BazaarDataOrigin.OrdersScreen origin) {
        ...
        return new ParsedEntry(info.get(), item, correctedFill, claimableAmount, attribution, observedStatus, Optional.ofNullable(finalExpiresAt));
    }

    public static OrderStatus.FillState resolveFillState(int filledAmount, int volume, long observedAt) {
    ...
    private static OrderStatus resolveObservedStatus(int filledAmount, int volume, boolean expired, long observedAt) {
        var progress = resolveFillState(filledAmount, volume, observedAt);

        return expired ? new OrderStatus.Expired(observedAt, progress) : (OrderStatus) progress;
    }
