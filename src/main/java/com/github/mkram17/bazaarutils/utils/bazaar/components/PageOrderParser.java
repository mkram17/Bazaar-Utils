 ...
 *
 * <p>Screen lore quirk handling ({@link #resolveScreenFill},
 * {@link #resolveRawClaimableUnits}) lives here because it has exactly one
...
public final class PageOrderParser {
     ...
     */
    public record ParsedEntry(OrderInfo info, ItemInfo item, int filledAmount, int claimableAmount) {
    ...
    private static @Nullable ParsedEntry parseEntry(ItemInfo item) {
        ...
        return new ParsedEntry(info.get(), item, correctedFill, claimableAmount);
    }
    ...
    private static int resolveRawClaimableUnits(TransactionType.Side side, @Nullable String claimStr, double pricePerItem) {
        ...
        double tax = BUConfig.USER_BAZAAR_FLIPPER_ACCOUNT_UPGRADE.getUserBazaarTax() / 100.0;
