...
public final class UserOrderHandler extends BUListener {
    ...
    /**
     * Evaluates position for all active orders in the changed product set, then prunes
     * {@code lastKnown} entries for orders no longer present in storage.
     */
    ...
    /**
     * Computes the current {@link Order.PositionContext} and fires
     * {@link UserOrderPositionEvent} if it differs from the last known value.
     * {@link java.util.Map#put} returns the displaced entry, providing the previous context for
     * transition detection without a separate lookup.
     */
    private void checkOrder(Order order, List<Order> userOrders) {
        ...
        new UserOrderPositionEvent(order, current, previous).post(EVENT_BUS);
    }
}