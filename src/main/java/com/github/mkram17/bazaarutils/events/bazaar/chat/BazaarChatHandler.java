...
/**
 * Matches incoming chat messages against Bazaar event patterns and posts the corresponding
 * {@link BazaarChatEvent} subtype.
 *
 * <p>Owns pattern matching and raw value extraction only. Handler methods extract the
 * literal captured groups — display names, coin totals, item counts — and post the event
 * with no further interpretation. Product ID resolution, tax arithmetic, price derivation,
 * and order matching are deferred to the consuming data sources.
 *
 * <p>Messages containing the literal string {@code "Error"} are filtered before pattern
 * matching to prevent Hypixel error responses (e.g. "Error: Not enough coins!") from
 * partially matching placement or cancellation patterns.
 *
 * <p>{@link com.github.mkram17.bazaarutils.utils.RegexSwitch} is evaluated in declaration
 * order. {@link #BUY_ORDER_FLIPPED} is checked first because its pattern is a strict superset
 * of none of the others; the ordering exists to prevent a shorter pattern from consuming
 * a message that a later, more specific pattern would match correctly.
 */
@Module
public class BazaarChatHandler extends BUListener {
    ...
    public void onChat(ChatReceivedEvent.Pre event) {
        ...
        RegexSwitch.when()
                .on(BUY_ORDER_FLIPPED, BazaarChatHandler::handleBuyOrderFlip)
    ...
    private static void handleBuyOrderCreated(Matcher matcher) {
        ...
        post(new BazaarChatEvent.BuyOrderCreated(now(), removeFormatting(matcher.group("item")), coins(matcher.group("price")), amount(matcher.group("amount"))));
    }
    ...
    private static void handleSellOfferCreated(Matcher matcher) {
        ...
        post(new BazaarChatEvent.SellOfferCreated(now(), removeFormatting(matcher.group("item")), coins(matcher.group("price")), amount(matcher.group("amount"))));
    }
    ...
    private static void handleSellOfferCancelled(Matcher matcher) {
        ...
        post(new BazaarChatEvent.SellOfferCancelled(now(), removeFormatting(matcher.group("item")), amount(matcher.group("amount"))));
    }
    ...
    private static void handleFilledBuyOrder(Matcher matcher) {
        ...
        post(new BazaarChatEvent.BuyOrderFilled(now(), removeFormatting(matcher.group("item")), amount(matcher.group("amount"))));
    }
    ...
    private static void handleFilledSellOffer(Matcher matcher) {
        ...
        post(new BazaarChatEvent.SellOfferFilled(now(), removeFormatting(matcher.group("item")), amount(matcher.group("amount"))));
    }
    ...
    private static void handleBuyOrderClaim(Matcher matcher) {
        ...
        post(new BazaarChatEvent.BuyOrderClaimed(now(), removeFormatting(matcher.group("item")), amount(matcher.group("amount")), coins(matcher.group("coins")), coins(matcher.group("price"))));
    }
    ...
    private static void handleSellOfferClaim(Matcher matcher) {
        ...
        post(new BazaarChatEvent.SellOfferClaimed(now(), removeFormatting(matcher.group("item")), amount(matcher.group("amount")), coins(matcher.group("coins")), coins(matcher.group("price"))));
    }
    ...
    private static void handleInstantBuyAction(Matcher matcher) {
        ...
        post(new BazaarChatEvent.InstantBuy(now(), removeFormatting(matcher.group("item")), coins(matcher.group("price")), amount(matcher.group("amount"))));
    }
    ...
    private static void handleInstantSellAction(Matcher matcher) {
        ...
        post(new BazaarChatEvent.InstantSell(now(), removeFormatting(matcher.group("item")), coins(matcher.group("coins")), amount(matcher.group("amount"))));
    }

    private static final Pattern BUY_ORDER_FLIPPED = Pattern.compile("Order Flipped! (?<amount>[\\d,]+)x (?<item>.+?) for (?<profit>-?[\\d,.]+) coins of total expected profit");
    ...
    private static void handleBuyOrderFlip(Matcher matcher) {
        ...
        post(new BazaarChatEvent.BuyOrderFlipped(now(), removeFormatting(matcher.group("item")), amount(matcher.group("amount")), coins(matcher.group("profit"))));
    }
