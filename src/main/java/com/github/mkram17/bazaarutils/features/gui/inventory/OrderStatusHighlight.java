...
public class OrderStatusHighlight implements LoreModifier, SlotHighlight {
    ...
    private sealed interface HighlightState permits HighlightState.Unsettled, HighlightState.Settled {
        record Unsettled(PricingPosition position) implements HighlightState {}
        record Settled(OrderStatus status) implements HighlightState {}

        static Optional<HighlightState> create(Order order) {
            ProfileKey key = ProfileKey.requireProfile("OrderStatusHighlight");
    ...
    public Result modifyLore(ItemStack stack, List<Component> lore, @Nullable Result previous, @Nullable ScreenContext context) {
        ...
        return withMerger(lore, merger -> {
            ...
            switch (highlight) {
                case HighlightState.Settled it -> {
                    switch (it.status()) {
                        case OrderStatus.Filled ignored -> merger.add(styledText("FILLED", InventoryConfig.ORDER_STATUS_HIGHLIGHT_SETTLED_COLOR, true));
                        case OrderStatus.Expired ignored -> merger.add(styledText("EXPIRED", InventoryConfig.ORDER_STATUS_HIGHLIGHT_SETTLED_COLOR, true));
                        default -> {}
                    }
                }
                case HighlightState.Unsettled state -> {
    ...
    private static Optional<HighlightState> resolveHighlight(Order order) {
        ProfileKey key = ProfileKey.requireProfile("OrderStatusHighlight"); if (key == null) return Optional.empty();
        var storage = UserOrdersStorage.orders(key);
    ...
    private static int colorFor(HighlightState state) {
        return switch (state) {
            case HighlightState.Unsettled unsettled -> switch (unsettled.position()) {
            ...
            case HighlightState.Settled ignored -> InventoryConfig.ORDER_STATUS_HIGHLIGHT_SETTLED_COLOR;
        };
    }
